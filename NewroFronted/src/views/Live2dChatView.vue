 ...
 */
const handleCharacterTextResponse = (text: string , hasAudio = false, audioData = '') => {
  const emotionSeparatorIndex = text.indexOf("|");
  let emotion = '';
  let actualText = text;
  
  if (emotionSeparatorIndex !== -1) {
    emotion = text.substring(0, emotionSeparatorIndex);
    actualText = text.substring(emotionSeparatorIndex + 1);
  }
  console.log("表情",emotion);
  console.log("文本",actualText);
  ...
  // 添加角色消息到聊天历史
  if(hasAudio==true){
    addCharacterMessage(actualText, true, audioData);
  }else{
    addCharacterMessage(actualText);
  }
...
const handleCharacterAudioResponse = (audioBase64: string, text?: string) => {
  // 如果同时收到了文本和语音，先显示文本
  if (text) {
    handleCharacterTextResponse(text, true, audioBase64)
  } else { // 这种情况不会出现
    handleCharacterTextResponse('[语音消息]', true, audioBase64)
  }
  // 只有在非静音状态下才自动播放语音
  if (!isMuted.value) {
